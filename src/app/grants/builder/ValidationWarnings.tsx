    ...
    <div className="space-y-3">
      ...
      {(hasErrors || hasWarnings) && (
        <div className="flex items-center gap-2 text-xs">
          {hasErrors && (
            <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-1 text-red-700 font-medium">
              <XCircle className="h-3 w-3" />
              {validation.errors.length} {validation.errors.length === 1 ? "Error" : "Errors"}
            </span>
          )}
          {hasWarnings && (
            <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 text-amber-700 font-medium">
              <AlertCircle className="h-3 w-3" />
              {validation.warnings.length} {validation.warnings.length === 1 ? "Warning" : "Warnings"}
            </span>
          )}
          {validation.infos.length > 0 && (
            <span className="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2 py-1 text-blue-700 font-medium">
