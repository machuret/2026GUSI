...
import {
  ...
  Play, Pencil, Save, Check, Copy, ArrowLeft,
} from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        {pagination && (
          <div className="rounded-lg bg-indigo-50 border border-indigo-200 px-4 py-2 text-sm font-medium text-indigo-700 shrink-0">
            {pagination.totalCount} transcription{pagination.totalCount !== 1 ? "s" : ""}
          </div>
        )}
      </div>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-lg">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search transcripts by title, description, or content…"
            className="w-full rounded-lg border border-gray-300 pl-9 pr-9 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-200" />
          {search && (
        ...
        {pagination && videos.length > 0 && (
          <span className="text-xs text-gray-400">
            Showing {videos.length} of {pagination.totalCount}
          </span>
        )}
      </div>
      ...
      {loading && (
        <div className="py-20 text-center text-gray-400">
          <Loader2 className="mx-auto h-8 w-8 animate-spin mb-3" /> Loading transcriptions…
        </div>
      )}
      ...
      {!loading && videos.length === 0 && (
        <div className="rounded-xl border border-dashed border-gray-300 py-20 text-center">
          ...
          <p className="font-medium text-gray-500">
            {debouncedSearch ? "No transcriptions match your search" : "No transcriptions available yet"}
          </p>
          <p className="mt-1 text-sm text-gray-400">
            {debouncedSearch
              ? "Try a different search term."
              : "Go to the Videos page and click \"Fetch Transcripts\" to download them from Vimeo."}
          </p>
          {!debouncedSearch && (
            <Link href="/videos"
              className="mt-4 inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">
              <Play className="h-4 w-4" /> Go to Videos
            </Link>
          )}
        </div>
      )}
                ...
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <button onClick={() => setExpandedId(isExpanded ? null : v.id)}
                    ...
                    <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">
                      {v.transcript.length > 140 ? v.transcript.slice(0, 140).trimEnd() + "…" : v.transcript}
                    </p>
                  ...
                  <div className="border-t border-gray-100 px-4 py-3 bg-gray-50">
                    {isEditing ? (
                      ...
                      <div>
                        ...
                        <pre className="whitespace-pre-wrap text-xs text-gray-700 leading-relaxed max-h-96 overflow-y-auto rounded-lg bg-white border border-gray-200 p-3">
                          {v.transcript}
                        </pre>
                      </div>
                    )}
