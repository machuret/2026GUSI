...
const STATUS_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  PUBLISHED: { label: "Published", cls: "bg-purple-100 text-purple-700 border border-purple-200" },
};

interface Props {
  item: ContentWithMeta & { isEdited?: boolean; scheduledAt?: string | null; destinations?: string[] };
  ...
  onDestinationsChanged?: (id: string, destinations: string[]) => void;
}

function LibraryCardInner({
  item, onPublish, onEdit, onDelete, onChangeCategory, onScheduled, onDestinationsChanged,
}: Props) {
  ...
  const [destinations, setDestinations]       = useState<string[]>(item.destinations ?? []);
  ...
  const handleChangeCategory = async (newCat: string) => {
    setActionLoading(true);
    try { await onChangeCategory(item.id, item.category, newCat); }
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden transition-shadow hover:shadow-sm">
      {/* ── Top meta bar ── */}
      <div className="flex items-center justify-between gap-3 border-b border-gray-100 bg-gray-50 px-4 py-2.5 flex-wrap">
      ...
      <div className="p-5">
        {/* Prompt */}
        <p className="mb-3 text-xs text-gray-400 italic truncate">
        ...
        {!editMode && (
          <div className="mt-3">
            <DestinationPicker
              ...
              onSaved={(d) => {
                setDestinations(d);
                onDestinationsChanged?.(item.id, d);
              }}
            />
          </div>
        )}
        ...
        {/* ── Scheduled indicator ── */}
        {scheduledAt && !editMode && (
          <p className="mt-2 text-xs text-blue-600 flex items-center gap-1">
            <Calendar className="h-3 w-3" />
            Scheduled: {new Date(scheduledAt).toLocaleDateString("en-AU", {
